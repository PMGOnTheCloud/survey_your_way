            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden">
                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    ...
                    <div class="mt-1 flex items-center">
                        ...
                        <button type="button" class="[&>*]:cursor-pointer relative overflow-hidden ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            <input type="file" @change="onImageChoose" class="absolute left-0 top-0 right-0 bottom-0 opacity-0" />
                            Change
                        </button>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea id="description" name="description" rows="3" v-model="model.description" autocomplete="survey_description" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="Describe your survey" />
                        </div>
                    </div>
                    <div>
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 text-sm">
                            <label for="status" class="font-medium tet-gray-700">
                                Active
                            </label>
                        </div>
                    </div>
...
<script setup>
    ...
    function saveSurvey() {
        store.dispatch("saveSurvey", model.value).then(({data}) => {
            store.commit('notify', {
                type: 'success',
                message: 'Survey was succesfully saved'
            });
            router.push({
                name: "SurveyView",
    ...
    function deleteSurvey() {
        if (confirm(`Are you sure you want to delete this survey? This survey can't be recovered`)) {
